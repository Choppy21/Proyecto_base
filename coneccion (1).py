...
def ventana_materias(alumno_id):
    ...
    materias_window.geometry("400x600")
    materias_window.configure(bg="#2C3E50")

    tk.Label(materias_window, text="Materias Inscritas", font=("Arial", 20, "bold"), fg="white", bg="#2C3E50").place(relx=0.5, rely=0.1, anchor="center")

    listbox_inscritas = tk.Listbox(materias_window, font=("Arial", 12), bg="#ECF0F1", fg="black")
    listbox_inscritas.place(relx=0.5, rely=0.25, anchor="center", width=250, height=120)

    tk.Label(materias_window, text="Selecciona materias nuevas", font=("Arial", 20, "bold"), fg="white", bg="#2C3E50").place(relx=0.5, rely=0.45, anchor="center")
    listbox_materias = tk.Listbox(materias_window, selectmode=tk.MULTIPLE, font=("Arial", 12), bg="#ECF0F1", fg="black")
    listbox_materias.place(relx=0.5, rely=0.6, anchor="center", width=250, height=120)

    def actualizar_materias_inscritas():
        """ Consulta y muestra las materias en las que ya está inscrito el alumno. """
        ...
        for materia in materias_inscritas:
            listbox_inscritas.insert(tk.END, materia[0])
    ...
    def guardar_inscripcion():
        """ Inscribir al alumno en materias nuevas. """
    ...
    tk.Button(materias_window, text="Inscribirse en nuevas materias", font=("Arial", 15, "bold"), bg="#E74C3C", fg="white", command=guardar_inscripcion).place(relx=0.5, rely=0.80, anchor="center")
    tk.Button(materias_window, text="Volver al Menú", font=("Arial", 15, "bold"), bg="#3498DB", fg="white", command=lambda: volver_menu(materias_window)).place(relx=0.5, rely=0.90, anchor="center")
